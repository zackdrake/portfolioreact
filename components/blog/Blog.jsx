...
const Blog = () => {
  ...
  const blogsData = getBlogsData(t);
  ...
  const handleData = (text) => {
    if (text === "All") {
      ...
      const findData = blogsData.filter((item) => item.category === text);
      setData(findData);
    }
  };
  ...
  return (
    <>
      ...
      <ul className="mt-[40px] flex w-full justify-start md:justify-end flex-wrap font-medium pb-12">
        ...
        <li
            className={`${
                test === t('proficiencyHuman') ? "text-[#FA5252]" : "fillter-btn"
            } mr-4 md:mx-4`}
            onClick={() => handleSearch(t('proficiencyHuman'))}
        >
          {t('proficiencyHuman')}
        </li>
        <li
            className={`${
                test === t('proficiencyTechnical') ? "text-[#FA5252]" : "fillter-btn"
            } mr-4 md:mx-4`}
            onClick={() => handleSearch(t('proficiencyTechnical'))}

        >
          {t('proficiencyTechnical')}
        </li>
      </ul>
